...
class WC_Gateway_FirstAtlanticCommerce extends WC_Payment_Gateway
{
    ...
    public function validate_fields()
    {
        ...
        if ( empty($_POST['expdatemonth']) )
        {
            wc_add_notice( $this->get_validation_error( __('Expiry Month', 'woocommerce-gateway-fac'), $_POST['fac-card-number'] ), 'error' );
            $validated = false;
        }
        if ( empty($_POST['expdateyear']) )
        {
            wc_add_notice( $this->get_validation_error( __('Expiry year', 'woocommerce-gateway-fac'), $_POST['fac-card-number'] ), 'error' );
            $validated = false;
        }
    ...
    public function process_payment($order_id)
    {
        ...
        try
        {
            ...
            $GLOBALS['merchantResponseURL'] = $this->get_return_url($order);
            ...
            else
            {
                ...
                $card_expiry_month = wc_clean($_POST['expdatemonth']);
                $card_expiry_year = wc_clean($_POST['expdateyear']);
            ...
            if ( $response->isSuccessful() )
            {
                ...
                update_post_meta($order_id, "order_fac_form", $res_data['HTMLFormData']);
